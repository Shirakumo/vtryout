...
(define-handler (actor face-update) (local-transform packet)
  (let ((pose (actor-pose actor)))
    (macrolet ((update (via place new &optional (rate 0.5))
      ...
      (update lerp (blink-left pose) (max -0.65 (+ -1.0 (packet-blink-left packet))))
      (update lerp (blink-right pose) (max -0.65 (+ -1.0 (packet-blink-right packet))))
      (update lerp (eye-left pose) (qfrom-angle +vx+ (- (* 0.5 (packet-eye-left packet)) 0.4)))
      (update lerp (eye-right pose) (qfrom-angle +vx+ (- (* 0.5 (packet-eye-right packet)) 0.4)))
      (let ((freq (vz (main-frequency (harmony:segment 'speech-detection T)))))
        (when (< 0 freq)
          (update lerp (jaw-open pose) (* 2 freq)))))))
